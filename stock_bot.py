...
class CZaraStockBot:
    ...
    def __init__(self):
        ...
        # Eğer BOT_TOKEN ortam değişkeni ayarlanmamışsa, bir ValueError hatası verir.
        # Bu, botu çalıştırmadan önce token'ı ayarlamanız gerektiğini garanti eder.
        bot_token = os.environ.get('BOT_TOKEN')
        if not bot_token:
            raise ValueError("BOT_TOKEN ortam değişkeni ayarlanmamış!")

        self.updater = Updater(token=bot_token) # <-- Burayı değiştirdik
        self.bot = telegram.Bot(token=bot_token) # <-- Burayı değiştirdik
